(() => {
  ...
  if (typeof parseMidiTrackData === 'function') {
    window.parseMidiTrackData = function parseMidiTrackDataFixed(data) {
      const events = [];
      let pointer = 0;
      let absTime = 0;
      let runningStatus = 0;
      ...
      while (pointer < data.length) {
        ...
        if (hi === 0x90) {
          const note = data[pointer];
          const velocity = data[pointer + 1];
          events.push({
            absTime,
  ...
  if (typeof generateForChord === 'function') {
    const originalGenerateForChord = generateForChord;
    window.generateForChord = function generateForChordFixed(chord, barCount, barOffset, shared) {
      const buildSharedForBar = currentBarOffset => {
        let nextShared = shared;
        if (chord &&
            chord.iv &&
            typeof JI_PARAMS !== 'undefined' &&
            JI_PARAMS &&
            JI_PARAMS.enabled &&
            typeof JustIntonation !== 'undefined') {
          const safePbRange =
            typeof S !== 'undefined' && S && Number.isFinite(S.pbRange) && S.pbRange > 0
              ? S.pbRange
              : 2;
          const jiParamsForBar = typeof PATCH.getJiParamsForBar === 'function'
            ? PATCH.getJiParamsForBar(currentBarOffset)
            : JI_PARAMS;
          nextShared = Object.assign({}, shared, {
            jiPBMap: typeof PATCH.buildAdaptiveJiPitchBendMap === 'function'
              ? PATCH.buildAdaptiveJiPitchBendMap(chord, safePbRange, currentBarOffset)
              : JustIntonation.getPitchBendMap(
                  chord.iv.map(interval => (((chord.root % 12) + 12 + interval) % 12 + 12) % 12),
                  ((chord.root % 12) + 12) % 12,
                  jiParamsForBar,
                  safePbRange
                ),
      ...
      const segments = typeof PATCH.getCompSectionSegments === 'function'
        ? PATCH.getCompSectionSegments(barOffset, barCount)
        : [{ barOffset, bars: barCount }];
      if (!Array.isArray(segments) || segments.length <= 1) {
        const nextShared = buildSharedForBar(barOffset);
        const result = originalGenerateForChord(chord, barCount, barOffset, nextShared);
        if (typeof PATCH.isCompSectionRuntimeActive === 'function' && PATCH.isCompSectionRuntimeActive()) {
          ...
          result.tracks = PATCH.applyCompTuningToTrackSlice(result.tracks, result.trackNames, chord, barOffset);
        }
        return result;
      }
      ...
      const segmentResults = segments.map(segment => {
        ...
        const result = originalGenerateForChord(chord, segment.bars, segment.barOffset, nextShared);
        result.tracks = PATCH.applyCompTuningToTrackSlice(result.tracks, result.trackNames, chord, segment.barOffset);
        return {
  ...
  if (typeof applyCompensationTuningToMidi === 'function') {
    const originalApplyCompensationTuningToMidi = applyCompensationTuningToMidi;
    window.applyCompensationTuningToMidi = function applyCompensationTuningToMidiFixed(midiData, pbRange) {
      if (!window.__MEIKY_ALLOW_COMP_SECTION_INTERNAL__ &&
          typeof S !== 'undefined' &&
          S &&
          S.compSectionPreTuned) {
        return midiData;
      }
      let effectivePbRange = pbRange;
      if (typeof CompTuningEngine !== 'undefined' &&
        ...
        effectivePbRange = CompTuningEngine.STATE.pbRange;
      }
      return originalApplyCompensationTuningToMidi(midiData, effectivePbRange);
    };
  }

  if (typeof buildMPEConductor === 'function') {
    window.buildMPEConductor = function buildMPEConductorFixed(bpm, ppq, bars, tensionCurve, tempoEnabled, tempoDrift, sections, pbRange) {
      const safePbRange = Number.isFinite(pbRange) && pbRange > 0 ? pbRange : 2;
      const barTicks = ppq * 4;
      const pool = [];
      const tempoUs = Math.round(60000000 / bpm);
      const tempoBase = { tick: 0, bytes: [0xFF, 0x51, 3, (tempoUs >> 16) & 0xFF, (tempoUs >> 8) & 0xFF, tempoUs & 0xFF] };
      pool.push(tempoBase);
      pool.push({ tick: 0, bytes: [0xB0, 101, 0] });
      pool.push({ tick: 0, bytes: [0xB0, 100, 6] });
      pool.push({ tick: 0, bytes: [0xB0, 6, safePbRange] });
      pool.push({ tick: 0, bytes: [0xB0, 38, 0] });
      for (let channel = 1; channel <= 15; channel++) {
        pool.push({ tick: 0, bytes: [0xB0 | channel, 101, 0] });
        pool.push({ tick: 0, bytes: [0xB0 | channel, 100, 0] });
        pool.push({ tick: 0, bytes: [0xB0 | channel, 6, safePbRange] });
        pool.push({ tick: 0, bytes: [0xB0 | channel, 38, 0] });
      }
      if (sections && sections.length) {
        for (const section of sections) {
          const tick = section.bar * barTicks;
          const nameBytes = [...section.name].map(char => char.charCodeAt(0));
          pool.push({ tick: Math.max(1, tick), bytes: [0xFF, 0x06, nameBytes.length, ...nameBytes] });
        }
      }
      if (tempoEnabled && tensionCurve && tensionCurve.length > 0) {
        for (let bar = 0; bar < bars; bar++) {
          const tick = bar * barTicks;
          const tension = tensionCurve[bar] != null ? tensionCurve[bar] : 0.5;
          const currentBpm = Math.max(15, Math.min(200, Math.round(bpm + (tension - 0.5) * 2 * tempoDrift)));
          const currentUs = Math.round(60000000 / currentBpm);
          if (bar === 0) {
            tempoBase.bytes = [0xFF, 0x51, 3, (currentUs >> 16) & 0xFF, (currentUs >> 8) & 0xFF, currentUs & 0xFF];
          } else {
            pool.push({ tick, bytes: [0xFF, 0x51, 3, (currentUs >> 16) & 0xFF, (currentUs >> 8) & 0xFF, currentUs & 0xFF] });
          }
        }
      }
      pool.sort((a, b) => a.tick - b.tick);
      const bytes = [];
      let previousTick = 0;
      for (const event of pool) {
        bytes.push(...vl(event.tick - previousTick), ...event.bytes);
