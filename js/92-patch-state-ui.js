(() => {
  ...
  if (typeof StatePersist !== 'undefined' &&
      typeof StatePersist.save === 'function' &&
      typeof StatePersist.load === 'function') {
    const originalStatePersistSave = StatePersist.save.bind(StatePersist);
    const originalStatePersistLoad = StatePersist.load.bind(StatePersist);
    const originalStatePersistExportAll = typeof StatePersist.exportAll === 'function'
      ? StatePersist.exportAll.bind(StatePersist)
      : null;
    const originalStatePersistImportAll = typeof StatePersist.importAll === 'function'
      ? StatePersist.importAll.bind(StatePersist)
      : null;

    StatePersist.save = function saveWithExtraState(slotName, label) {
      const ok = originalStatePersistSave(slotName, label);
      if (!ok) return ok;
      try {
        const raw = localStorage.getItem(PATCH.STORAGE_PREFIX + slotName);
        if (!raw) return ok;
        const snapshot = JSON.parse(raw);
        snapshot.ji = typeof JI_PARAMS !== 'undefined' ? PATCH.jsonClone(JI_PARAMS) : null;
        snapshot.rc = typeof RC_PARAMS !== 'undefined' ? PATCH.jsonClone(RC_PARAMS) : null;
        snapshot.compTuning = (typeof CompTuningEngine !== 'undefined' && CompTuningEngine.STATE)
          ? {
              ...PATCH.jsonClone(CompTuningEngine.STATE),
              channels: Array.from(CompTuningEngine.STATE.channels || []),
            }
          : null;
        snapshot.sectionLearnFocus = PATCH.jsonClone(PATCH.sectionLearnFocus);
        snapshot.sectionPreviewFollow = PATCH.jsonClone(PATCH.ensureSectionPreviewFollow());
        snapshot.midiCapturePolicy = PATCH.jsonClone(PATCH.ensureMidiCapturePolicy());
        snapshot.conductorViz = typeof CONDUCTOR_VIZ_STATE !== 'undefined' ? PATCH.jsonClone(CONDUCTOR_VIZ_STATE) : null;
        localStorage.setItem(PATCH.STORAGE_PREFIX + slotName, JSON.stringify(snapshot));
      } catch (_) {}
      return ok;
    };

    StatePersist.load = function loadWithExtraUISync(slotName) {
      let patchSnapshot = null;
      try {
        const raw = localStorage.getItem(PATCH.STORAGE_PREFIX + slotName);
        patchSnapshot = raw ? JSON.parse(raw) : null;
      } catch (_) {}

      const ok = originalStatePersistLoad(slotName);
      if (!ok) return ok;

      PATCH.clearGeneratedArtifacts();

      if (patchSnapshot && patchSnapshot.ji && typeof JI_PARAMS !== 'undefined') {
        Object.assign(JI_PARAMS, patchSnapshot.ji);
      }
      if (patchSnapshot && patchSnapshot.rc && typeof RC_PARAMS !== 'undefined') {
        Object.assign(RC_PARAMS, patchSnapshot.rc);
      }
      if (patchSnapshot && patchSnapshot.compTuning && typeof CompTuningEngine !== 'undefined' && CompTuningEngine.STATE) {
        const saved = { ...patchSnapshot.compTuning };
      ...
      if (patchSnapshot && patchSnapshot.sectionLearnFocus && PATCH.sectionLearnFocus) {
        Object.assign(PATCH.sectionLearnFocus, patchSnapshot.sectionLearnFocus);
      }
      if (patchSnapshot && patchSnapshot.sectionPreviewFollow && PATCH.sectionPreviewFollow) {
        Object.assign(PATCH.ensureSectionPreviewFollow(), patchSnapshot.sectionPreviewFollow);
      }
      if (patchSnapshot && patchSnapshot.midiCapturePolicy) {
        Object.assign(PATCH.ensureMidiCapturePolicy(), patchSnapshot.midiCapturePolicy);
      }
      if (patchSnapshot && patchSnapshot.conductorViz && typeof CONDUCTOR_VIZ_STATE !== 'undefined') {
        Object.assign(CONDUCTOR_VIZ_STATE, patchSnapshot.conductorViz);
      }
      ...
      if (typeof CompTuningEngine !== 'undefined' && CompTuningEngine.STATE) {
        ...
        const compTuningEnabled = typeof S !== 'undefined' && S && S.engines ? !!S.engines.comptuning : false;
        if (toggler) toggler.classList.toggle('on', compTuningEnabled);
        if (controls) controls.style.display = compTuningEnabled ? 'block' : 'none';

        const pbSlider = document.getElementById('ctPbRange');
        const pbValue = document.getElementById('ctPbRangeV');
        const a4Slider = document.getElementById('ctA4Slider');
        const a4Value = document.getElementById('ctA4Val');
        const globalSlider = document.getElementById('ctGlobalCents');
        const globalValue = document.getElementById('ctGlobalCentsV');
        if (pbSlider) pbSlider.value = CompTuningEngine.STATE.pbRange;
        if (pbValue) pbValue.textContent = '±' + CompTuningEngine.STATE.pbRange + ' st';
        if (a4Slider) a4Slider.value = CompTuningEngine.STATE.a4Hz;
        if (a4Value) a4Value.textContent = CompTuningEngine.STATE.a4Hz.toFixed(1) + ' Hz';
        if (globalSlider) globalSlider.value = CompTuningEngine.STATE.globalCents;
        if (globalValue) {
          const sign = CompTuningEngine.STATE.globalCents >= 0 ? '+' : '';
          globalValue.textContent = sign + CompTuningEngine.STATE.globalCents.toFixed(1) + '¢';
        }

        const rootSlider = document.getElementById('ctRootPc');
        const rootValue = document.getElementById('ctRootPcV');
        if (rootSlider) rootSlider.value = CompTuningEngine.STATE.rootPc;
        if (rootValue) rootValue.textContent = PATCH.NOTE_NAMES[CompTuningEngine.STATE.rootPc] || 'C';
        ...
        if (typeof CompTuningEngine.buildA4Presets === 'function') {
          CompTuningEngine.buildA4Presets();
        }
        PATCH.ensureScalaTemperamentEntry();
        PATCH.refreshScalaProfile(false);
        if (typeof CompTuningEngine.buildTemperamentButtons === 'function') {
          CompTuningEngine.buildTemperamentButtons();
        }
        if (typeof CompTuningEngine.buildIntervalGrid === 'function') {
          CompTuningEngine.buildIntervalGrid();
        }
        if (typeof CompTuningEngine.updateDisplay === 'function') {
    ...
    if (originalStatePersistExportAll) {
      StatePersist.exportAll = function exportAllWithPatchExtras() {
        const slots = typeof StatePersist.listSlots === 'function' ? StatePersist.listSlots() : [];
        const payload = {
          version: 25,
          exportedAt: new Date().toISOString(),
          slots: [],
          patchExtras: {
            scalaFavorites: PATCH.readScalaFavorites(),
            ...
            midiCapturePolicy: PATCH.jsonClone(PATCH.ensureMidiCapturePolicy()),
          },
        };
        slots.forEach(slot => {
          const raw = localStorage.getItem(PATCH.STORAGE_PREFIX + slot.name);
          if (!raw) return;
          try {
            payload.slots.push({ name: slot.name, data: JSON.parse(raw) });
          } catch (_) {}
        });
        ...
        anchor.href = url;
        anchor.download = 'meikyoshisui_presets_' + new Date().toISOString().slice(0, 10) + '.json';
        anchor.click();
        URL.revokeObjectURL(url);
      };
    }

    if (originalStatePersistImportAll) {
      StatePersist.importAll = function importAllWithPatchExtras(onComplete) {
        ...
        input.addEventListener('change', () => {
          ...
          reader.onload = event => {
            ...
            parsed.slots.forEach(slot => {
              if (!slot.name || !slot.data) return;
              try {
                localStorage.setItem(PATCH.STORAGE_PREFIX + slot.name, JSON.stringify(slot.data));
                let index = typeof StatePersist.listSlots === 'function'
                  ? StatePersist.listSlots().filter(entry => entry.name !== slot.name)
                  : [];
                index.unshift({
                  name: slot.name,
                  savedAt: slot.data.savedAt || new Date().toISOString(),
                  label: slot.data.label || slot.name,
                });
                if (index.length > 20) index = index.slice(0, 20);
                localStorage.setItem(PATCH.STORAGE_PREFIX + 'slots', JSON.stringify(index));
                count++;
              } catch (_) {}
            });
            if (parsed.patchExtras &&
                parsed.patchExtras.scalaFavorites &&
                typeof parsed.patchExtras.scalaFavorites === 'object') {
              PATCH.writeScalaFavorites(parsed.patchExtras.scalaFavorites);
            }
            if (parsed.patchExtras &&
                parsed.patchExtras.midiLearnMappings &&
                typeof parsed.patchExtras.midiLearnMappings === 'object') {
              PATCH.writeMidiLearnMappings(parsed.patchExtras.midiLearnMappings);
            }
            if (parsed.patchExtras &&
                parsed.patchExtras.sectionLearnFocus &&
                typeof parsed.patchExtras.sectionLearnFocus === 'object' &&
                PATCH.sectionLearnFocus) {
              Object.assign(PATCH.sectionLearnFocus, parsed.patchExtras.sectionLearnFocus);
            }
            if (parsed.patchExtras &&
                parsed.patchExtras.sectionPreviewFollow &&
                typeof parsed.patchExtras.sectionPreviewFollow === 'object' &&
                PATCH.sectionPreviewFollow) {
              Object.assign(PATCH.ensureSectionPreviewFollow(), parsed.patchExtras.sectionPreviewFollow);
            }
            if (parsed.patchExtras &&
                parsed.patchExtras.midiCapturePolicy &&
                typeof parsed.patchExtras.midiCapturePolicy === 'object') {
              Object.assign(PATCH.ensureMidiCapturePolicy(), parsed.patchExtras.midiCapturePolicy);
            }
            PATCH.syncScalaUI();
            PATCH.syncJustIntonationUI();
            PATCH.syncCompSectionUI();
